...
function init.daemonize() {
	daemonize -v -c "$PWD" /usr/bin/env -i PATH=$PATH HOME=$HOME USER=$USER LANG=$LANG LC_ALL=$LC_ALL TERM=$TERM SHELL=$(which -p bash) $(which -p bash) -l -c "$*"
} && compdef init.daemonize=command
function init.daemonize.log() {
	local base="$(basename "$1")"
	daemonize -v -c "$PWD" -e "$HOME/.daemonize/$base.log" -o "$HOME/.daemonize/$base.log" /usr/bin/env -i PATH=$PATH HOME=$HOME USER=$USER LANG=$LANG LC_ALL=$LC_ALL TERM=$TERM SHELL=$(which -p bash) $(which -p bash) -l -c "$*"
