...
function init.daemonize.log() {
	...
	daemonize -v -c "$PWD" -e "$HOME/.daemonize/$base.log" -o "$HOME/.daemonize/$base.log" /usr/bin/env -i HOME=$HOME USER=$USER LANG=$LANG LC_ALL=$LC_ALL TERM=$TERM SHELL=$(which -p bash) $(which -p bash) -l -c "$*"
