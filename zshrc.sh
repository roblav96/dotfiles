...
function histw() {
	hist | rg --fixed-strings --case-sensitive --word-regexp "$*" | bat --color=always --style=grid -l bash | rg --fixed-strings --case-sensitive --passthru "$*"
}; compdef histw=which
...
test -x "$(which -p just)" && source "$DOTFILES/modules/just.sh"
