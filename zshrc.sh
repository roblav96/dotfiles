...
alias ll="ls -lAFhN"
alias diff="diff -u -x '.git' -x 'node_modules' -x 'package-lock.json'"
...
which aichat &>/dev/null && alias ai="aichat"
which unftp &>/dev/null && alias unftp="unftp --auth-type anonymous"
...
alias dr="deno run --unstable --allow-all --no-check"
...
alias bak='[[ -d ~/.playground/"$(basename "$PWD")" ]] && rd ~/.playground/"$(basename "$PWD")"; mkdir ~/.playground/"$(basename "$PWD")" && f --no-ignore --exclude=.next --exclude=.turbo --exclude=dist --exclude=build -d1 -X cp -r -t ~/.playground/"$(basename "$PWD")"'
...
alias rsync="rsync --ipv4 --verbose --human-readable --progress --archive --copy-links"
...
test -x "$(which -p ollama)" && source "$DOTFILES/modules/ollama.sh"
