...
alias bat="bat --italic-text=always --decorations=always --tabs=0 --paging=$BAT_PAGER --wrap=never --theme='$BAT_THEME' --style=header,grid"
...
function batlangs() {
	...
	local langs=('ActionScript' 'AppleScript' 'AWK' 'Bash' 'CMake' 'Erlang' 'F#' 'Generic Config' 'Haskell' 'INI' 'Julia' 'Kotlin' 'Lisp' 'Lua' 'Makefile' 'Manpage' 'Nim' 'Nix' 'OCaml' 'Pascal' 'Perl' 'Java Properties' 'Python' 'QML' 'Ruby' 'Strace' 'Stylus' 'Swift' 'Tcl' 'TOML' 'Verilog' 'YAML')
