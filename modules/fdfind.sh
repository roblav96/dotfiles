...
if [[ "$PLATFORM" == "Linux" ]]; then
	alias fr="fd $FD_FLAGS --no-ignore --full-path --fixed-strings --absolute-path --base-directory=/ --exclude=$HOME/.npm --exclude=$HOME/.pnpm-store --exclude=$HOME/.linuxbrew --exclude=$HOME/emby --exclude=/dev --exclude=/proc --exclude=/sys --exclude=/var/www --exclude=/var/lib/emby/cache --exclude=/var/lib/emby/metadata"
fi
