export RG_FLAGS="--color=always --heading --line-number --trim --no-messages --smart-case --hidden --fixed-strings --max-columns=\$(tput cols) --max-columns-preview --glob='!.git' --glob='!.DS_Store'"
