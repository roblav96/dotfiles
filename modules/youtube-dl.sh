...
alias ytsc='youtube-dl --output="%(uploader)s/%(uploader)s__%(title)s__%(view_count)s_%(like_count)s_.%(ext)s" --add-header="$SOUNDCLOUD_OAUTH" --extract-audio --add-metadata --embed-thumbnail'

# alias ytsc="youtube-dl \
# --output='%(uploader)s/%(uploader)s__%(title)s.%(ext)s' \
# --add-header=\"\$SOUNDCLOUD_OAUTH\" \
# --extract-audio --add-metadata --embed-thumbnail"
# # --audio-format=m4a \
